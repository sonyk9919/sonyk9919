...
### JobSync · 채용 공고 마감일 관리 도구 · [GitHub](https://github.com/sonyk9919/JobSync) · [Demo](https://job-sync-five.vercel.app)

공고 페이지를 HTML로 저장 후 드롭하면 마감일을 파싱해 Google Calendar에 등록하고 칸반 보드로 관리하는 개인 프로젝트.

- JSON-LD 우선 파싱 전략 + `AbstractParser` / `ParserFactory` 패턴으로 플랫폼 확장 구조 설계 (잡코리아·잡플래닛·원티드)
- Zod로 파서 output과 Calendar API 응답 양쪽 런타임 검증, 실제 HTML 없이 최소 Document 헬퍼 기반 단위 테스트 작성

`Next.js 16` `TypeScript` `Tailwind CSS v4` `TanStack Query` `Zustand` `Zod` `Vitest`

---

### 수달 · 분리배출 가이드 + 타이쿤 게임 서비스 · [GitHub](https://github.com/sudal-kgu)

분리배출 가이드의 낮은 리텐션을 보완하기 위해 타이쿤 게임을 결합한 서비스.

- 5인 팀 스프린트 계획 수립·스크럼 진행 주도 — 3개월간 일정 조율 및 전 과정 책임 (Project Lead)
- 클라우드 비용 최소화를 위해 온프레미스 리눅스 서버 직접 구축 (공인 IP + 포트포워딩), Jenkins CI/CD 파이프라인 설계
- 클라이언트 전체 개발 — React + Three.js로 3D 게임 화면 구현
- React + Spring + FastAPI(YOLO) + MQ 조합의 전체 시스템 아키텍처 설계
- API 서버 퀴즈·인증/인가 개발, 이미지 분석 서버 PoC 및 AI 모델 검증

`React` `Three.js` `Spring` `FastAPI` `YOLO` `MQ` `Jenkins` `Linux`
...
- 낙하 거리에 따라 파티클 개수·지속 시간 및 카메라 쉐이크 강도를 동적으로 조정해 물리 기반 시각 피드백 구현
- 템플릿 메서드 패턴 기반 7종 테트로미노, 셰이더 기반 배경·GSAP 애니메이션
